...
def save_combined_plot(train_data, test_data, title, folder, filename, label1='Train', label2='Test'):
    ...
    plt.title(title)
    plt.legend()
    plt.savefig(f"results_large_layer/{folder}/{filename}.png")
    plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--layer-sizes', nargs='+', type=int, default=[500000], help='List of layer sizes')
    ...
    for folder in folders:
        os.makedirs(f"results_large_layer/{folder}", exist_ok=True)
    ...
    for size in args.layer_sizes:
        ...
        torch.save(model.state_dict(), f'results_large_layer/model_weights_{size}.pth')
    ...
    with open('results_large_layer/results_data.json', 'w') as json_file:
        json.dump(results_data, json_file, indent=4)

    print("Accuracy and loss data saved to results_large_layer/results_data.json")
